...
static inline void mmio_insw(void __iomem *addr, u16 *dst, int count)
{
	...
	if (ofe_debug_pio) {
		...
		u16 data;
		while (--count >= 0) {
			printk(KERN_INFO "- Read Word %d from Addr 0x%p to 0x%p: ",
			       saved_count - count, addr, dst);
...
static inline void mmio_outsw(void __iomem *addr, const u16 *src, int count)
{
	...
	if (ofe_debug_pio) {
		int saved_count = count;
		while (--count >= 0) {
			printk(KERN_INFO "- Write Word %d from Addr 0x%p to 0x%p: ",
			       saved_count - count, src, addr);
