	...
	unsigned long start, unsigned long end)
{
	...
	/* JWF: this isn't right for the SMP case,
	 * but it is probably ok for the CMT case
	 */
	brcm_r4k_flush_cache_range(vma, start, end);
#else
	int exec = vma->vm_flags & VM_EXEC;
...
static void r4k_dma_cache_wback_inv(unsigned long addr, unsigned long size)
{
	...
	BUG_ON(size == 0);
	if (addr < KSEG0) {
		printk(KERN_ERR "%s bad addr 0x%08lx\n", __FUNCTION__, addr);
		dump_stack();
		r4k_blast_dcache();
		__sync();
		return;
...
void __cpuinit r4k_cache_init(void)
{
	...
	flush_cache_all		= r4k___flush_cache_all;
#else
