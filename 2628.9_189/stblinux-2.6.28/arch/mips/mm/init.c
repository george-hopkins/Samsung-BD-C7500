...
static __init int page_is_bmem(unsigned long pfn)
{
	int i;
	...
	for (i = 0; i < MAX_BMEM_REGIONS; i++) {
		...
		if (bmem_regions[i].paddr == 0)
			break;

		start = bmem_regions[i].paddr >> PAGE_SHIFT;
		end   = start + (bmem_regions[i].size >> PAGE_SHIFT);
...
#error "CONFIG_HIGHMEM and CONFIG_DISCONTIGMEM dont work together yet"
	...
	for (tmp = 0; tmp < max_low_pfn; tmp++) {
		...
		/* 
		 * skip PFNs that don't have a section memory map
		 */
		if (pfn_present(tmp) == 0) {
    #ifdef CONFIG_MIPS_BCM_NDVD
			/* we know are memory boundaries match our section size */
			tmp += PAGES_PER_SECTION;
			tmp &= PAGE_SECTION_MASK;
    #endif
			continue;
		}
#endif
...
#    endif
  ...
  #endif
		/*
		 * skip if there's no memory present
		 */
		if (!page_is_ram(tmp)) {
			SetPageReserved(page);
			continue;
