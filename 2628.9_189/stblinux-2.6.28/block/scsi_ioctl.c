...
void blk_set_cmd_filter_defaults(struct blk_cmd_filter *filter)
{
	...
	__set_bit(GPCMD_V0_ENABLE_DIAG_MODE, filter->read_ok);
	__set_bit(GPCMD_V0_GET_CHECKSUM, filter->read_ok);
	__set_bit(GPCMD_V1_REPORT_KEY, filter->read_ok);
	__set_bit(GPCMD_V1_READ_DVD_STRUCTURE, filter->read_ok);
	__set_bit(GPCMD_V2_READ_RECORDER, filter->read_ok);
#endif
...
#if defined (CONFIG_MIPS_BCM_NDVD)
	__set_bit(GPCMD_V0_FLASH_COMMAND, filter->write_ok);
	__set_bit(GPCMD_V1_SEND_KEY, filter->write_ok);
	__set_bit(GPCMD_V2_SEND_RECORDER, filter->write_ok);
#endif
}
...
static int cdb_xfer_len(unsigned char *cdb, unsigned int sector_size)
{
	...
	case GPCMD_V0_ENABLE_DIAG_MODE:
	case GPCMD_V0_GET_CHECKSUM:
	case GPCMD_V0_FLASH_COMMAND:
		xfer_len = 0;
		break;
	case REQUEST_SENSE:
		xfer_len = cdb[4];
	...
	case GPCMD_SEND_KEY:
	/* Vendor-unique opcodes that we allow */
	case GPCMD_V1_REPORT_KEY:
	case GPCMD_V1_SEND_KEY:
	case GPCMD_V1_READ_DVD_STRUCTURE:
	case GPCMD_V2_SEND_RECORDER:
		xfer_len = (cdb[8] << 8) | cdb[9];
		break;
	case GPCMD_V2_READ_RECORDER:
		xfer_len = (cdb[6] << 24) | (cdb[7] << 16) | (cdb[8] << 8) | cdb[9];
		break;
	...
	switch (cmd) {
		...
		case CDROM_SEND_PACKET: {
			...
			if ((cgc.timeout < 10*HZ ) &&
				 ...
				 * on all but READ_12.
				 */
				hdr.timeout = jiffies_to_msecs(10*HZ);
			}
			else
