...
static const unsigned long ata_eh_reset_timeouts[] = {
	...
	15000,	/* give > 10 secs of idleness for retarded devices */
#else
...
#if defined (CONFIG_MIPS_BCM_NDVD)
static unsigned int atapi_eh_request_sense(struct ata_device *dev, u8 *sense_buf, u8 dfl_sense_key);
#endif
#ifdef CONFIG_PM
...
void ata_scsi_error(struct Scsi_Host *host)
{
			...
			if (i < ATA_MAX_QUEUE) {
				...
				if (!(qc->flags & ATA_QCFLAG_FAILED)) {
					...
					if ((stat & (ATA_BUSY | ATA_DRDY | ATA_ERR)) == (ATA_DRDY | ATA_ERR)) {
						...
						stat = ap->ops->sff_check_status(ap);
	...
	if (issue_request_sense && sns_qc) {
		sns = atapi_eh_request_sense(sns_qc->dev,
					     sns_qc->scsicmd->sense_buffer,
					     sns_qc->result_tf.feature >> 4);
		if (!sns) {
	       ...
	       ata_postreset_fn_t postreset)
{
	...
	{
		/*
		 * We have been requested to NEVER offline a
		 * device.
		 */
		struct ata_device *dev;
