...
#if defined (CONFIG_MIPS_BCM_NDVD)
	/*
	 * SATA requires multiple-of-4 data transfer sizes
	 */
	char buffer[252];
#else
	char buffer[255];
...
int cdrom_open(struct cdrom_device_info *cdi, struct block_device *bdev, fmode_t mode)
{
	...
	invalidate_bdev(bdev->bd_inode->i_bdev);
#endif
...
#if defined (CONFIG_MIPS_BCM_NDVD)
	case CDROM_READ_CAPACITY: {
		long last = 0;
		cdinfo(CD_DO_IOCTL, "entering CDROM_READ_CAPACITY\n"); 
