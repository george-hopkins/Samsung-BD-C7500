		     ...
		     u_int cmd, u_long arg)
{
	...
	case MEMSETOOBSEL:
	{
		struct nand_oobinfo info;
		
		if (copy_from_user(&info, argp, sizeof(info)))
			return -EFAULT;
#ifdef CONFIG_MTD_BRCMNAND
        printk("%s : %s\n", __FUNCTION__, (info.useecc == MTD_NANDECC_OFF)? "MTD_NANDECC_OFF":"AUTOPLACE");
        if (info.useecc == MTD_NANDECC_OFF)
        {
            if (!mtd->disable_ecc)
    		{
	   		  ret = -EOPNOTSUPP;
...
if (ret) 
    ...
    case MTD_BRCMNAND_SCANBBT:
	{
		loff_t offs;

		if (copy_from_user(&offs, argp, sizeof(loff_t)))
			return -EFAULT;
		if (!mtd->scan_bbt_table)
		{
			//printk("MTD_BRCMNAND_READBBTCODE: mtd is: %08x mtd->get_bbt_code is: %08x\n", (unsigned int) mtd, (unsigned int) mtd->get_bbt_code);
			ret = -EOPNOTSUPP;
		}
		else
			return mtd->scan_bbt_table(mtd);
		break;
	}
	case MTD_BRCMNAND_NANDSTAT:
	{
		struct nand_statistics *stats = (struct nand_statistics *)argp;

		if (!mtd->get_statistics)
			ret = -EOPNOTSUPP;
		else
			return mtd->get_statistics(mtd, stats);
		break;
	}

	case MTD_BRCMNAND_DELETEBBT:
	{
		loff_t offs;

		if (copy_from_user(&offs, argp, sizeof(loff_t)))
			return -EFAULT;
		if (!mtd->delete_bbt_table)
		{
			//printk("MTD_BRCMNAND_READBBTCODE: mtd is: %08x mtd->get_bbt_code is: %08x\n", (unsigned int) mtd, (unsigned int) mtd->get_bbt_code);
			ret = -EOPNOTSUPP;
		}
		else
			return mtd->delete_bbt_table(mtd);
		break;
	}

	case MTD_BRCMNAND_READBBTCODE:
	{
		loff_t offs;
		...
		if (!mtd->get_bbt_code)
		{
			...
			return mtd->get_bbt_code(mtd, offs);
		break;
	}

	case MTD_BRCMNAND_WRITEBBTCODE:
	{

		struct nand_bbt_command command;

		if (copy_from_user(&command, argp, sizeof(struct nand_bbt_command)))
 			return -EFAULT;
		if (!mtd->set_bbt_code)
		{
			printk("MTD_BRCMNAND_WRITEBBTCODE: mtd is: %08x mtd->set_bbt_code is: %08x\n", (unsigned int) mtd, (unsigned int) mtd->set_bbt_code);
			ret = -EOPNOTSUPP;
		}
		else
			return mtd->set_bbt_code(mtd, command.offset, command.absolute_offset, command.code);
		break;
	}

	

	case MTD_BRCMNAND_GENECCERROR:
	{

		struct nand_generate_ecc_error_command *command = (struct nand_generate_ecc_error_command *)argp;

		if (!mtd->gen_ecc_error)
		{
			ret = -EOPNOTSUPP;
		}
		else
			return mtd->gen_ecc_error(mtd, command);
		break;
	}


#if defined(CONFIG_MIPS_BCM7401)
	case MTD_BRCMNAND_READNORFLASH:
	{
		...
		if (((unsigned int) buf.buff & 0x1) || (buf.offset & 0x1) || (buf.len & 0x1)) {
 		...
 		databuf = kmalloc(GFP_KERNEL, buf.len);
 		if (!databuf)
 			return -ENOMEM;
