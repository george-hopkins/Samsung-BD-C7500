		...
		if ((sshdr.asc == 0x04) && (sshdr.ascq == 0x01)) {
			...
			*/
			if (!scmd->device->not_ready) {
				...
				scmd->device->bsy_tmo = jiffies + HZ/10;
			}
				...
				*/
				sdev_printk(KERN_WARNING,  sdev,
					    "Drive taking too long to become ready - SIMULATE TRACK FOLLOWING ERROR\n\n");
...
int scsi_decide_disposition(struct scsi_cmnd *scmd)
{
	...
	case TASK_ABORTED:
		...
		*/
		...
		/*
		**  Note that the inclusion of Mechanism Status commands
		**  in this code block is a new wrinkle as of DVD_2.2.
		**  With re-architecture in user-level code, the
		**  Mechanism Status command is being issued in places
		**  it never was before. If a "not ready" condition
		**  is established, the inclusion of MECHANISM_STATUS in
		**  the if conditional below cases the "not_ready" flag
		**  to stay set for all MECHANISM_STATUS command completions.
		**  When a tray is auto-ejected for conditions other than
		**  ready timeout expiry, the not_ready flag will not
		**  be cleared in existing code flows. Without the else clause
		**  below it is therefore possible to get false loader ready
		**  timeout errors.
		**
		**  If the tray stays open for a long time, the ready timer
		**  will falsely trip when the tray is re-inserted. Therefore,
		**  clear the not_ready flag about a second after the ready
		**  timeout expiry for all command completions.
		*/
		if ( scmd->device->not_ready ) {
			if (scmd->cmnd[0] != REQUEST_SENSE &&
			    scmd->cmnd[0] != ALLOW_MEDIUM_REMOVAL &&
			    scmd->cmnd[0] != START_STOP &&
			    scmd->cmnd[0] != MECHANISM_STATUS) {
				scmd->device->not_ready = 0;
				scmd->device->rdy_tmo = 0;
				scmd->device->bsy_tmo = 0;
			}
			else if ( time_after(jiffies, scmd->device->rdy_tmo + HZ/4)) {
				scmd->device->not_ready = 0;
				scmd->device->rdy_tmo = 0;
				scmd->device->bsy_tmo = 0;
			}
