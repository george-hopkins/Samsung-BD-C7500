 ...
 * Description:
 **/
static int get_tray_state(struct scsi_device *sdev, unsigned char *result,
			  int buflen, struct scsi_sense_hdr *sshdr)
{
	unsigned char scsi_cmd[MAX_COMMAND_SIZE];

	scsi_cmd[0] = MECHANISM_STATUS;
	scsi_cmd[1] = 0;
	scsi_cmd[2] = 0;
	scsi_cmd[3] = 0;
	scsi_cmd[4] = 0;
	...
	return (scsi_execute_req(sdev, scsi_cmd, DMA_FROM_DEVICE, result, buflen /* size */, sshdr,
				 SCSI_TIMEOUT, 3));
}
...
static int scsi_start_unit(struct scsi_device *sdev, unsigned char *result,
			   unsigned int loejbits,  struct scsi_sense_hdr *sshdr)
{
	unsigned char scsi_cmd[MAX_COMMAND_SIZE];
	...
	scsi_cmd[4] = loejbits;
	scsi_cmd[5] = 0;
	return (scsi_execute_req(sdev, scsi_cmd, DMA_NONE, result, 0 /* size */, sshdr,
				 SCSI_TIMEOUT * 5, 3));
}
				  ...
				  void *hostdata)
{
	...
	if (res == SCSI_SCAN_LUN_PRESENT) {
		...
		if (sdev->do_early_spinup) {
			 ...
			 */
			do {
			  ret = get_tray_state(sdev, result, result_len, NULL);
			} while (ret && (retries-- > 0));
			...
			if (retries > 0) {
				...
				if (!tray_open) {
					 ...
					 */
					printk(KERN_INFO "scsi: Tray State CLOSED\n");
					do {
						ret = scsi_start_unit(sdev, result, 0x1, &sshdr);
						
						if ((driver_byte(ret) & DRIVER_SENSE) &&
						    scsi_sense_valid(&sshdr)) {
							/*
							 * If no disc is present, we're done.
							 */
							if ((sshdr.sense_key == NOT_READY) &&
							    ((sshdr.asc == 0x3a) &&
							     (sshdr.ascq == 0x00 || sshdr.ascq == 0x01)))
								break;
						}
					} while (ret && (retries-- > 0));

					if (ret)
						printk(KERN_INFO "scsi: Loader could not execute early spinup !\n");
					else
						printk(KERN_INFO "scsi: Early spinup performed\n");
				}
				else
					printk(KERN_INFO "scsi: Tray State OPEN\n");
