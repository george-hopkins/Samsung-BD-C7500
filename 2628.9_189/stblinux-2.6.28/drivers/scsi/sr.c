...
#if defined (CONFIG_MIPS_BCM_NDVD)
		{
			...
			if (scsi_sense_valid(sshdr)) {
				if (sshdr->sense_key == NOT_READY &&
					 ...
					 * if No Medium Present
					 */
					retries = 0;
				}
				else if (sshdr->sense_key == MEDIUM_ERROR)
					retries = 0;
			}
		}
#endif
...
static int sr_prep_fn(struct request_queue *q, struct request *rq)
{
...
#if defined (CONFIG_MIPS_BCM_NDVD)
		scmd_printk(KERN_NOTICE, SCpnt, "rq->sector   = 0x%x\n", (unsigned)rq->sector);
		scmd_printk(KERN_NOTICE, SCpnt, "s_size       = 0x%x\n", s_size    );
		scmd_printk(KERN_NOTICE, SCpnt, "scsi_bufflen = 0x%x\n", scsi_bufflen(SCpnt));
		dump_stack();
#endif
		goto out;
	}
	...
	/*
	** For READ_10/READ_12/READ_CD, limit retries.
	*/
	if (SCpnt->cmnd[0] == READ_10 ||
	    SCpnt->cmnd[0] == READ_12 ||
	    SCpnt->cmnd[0] == READ_CD)
		rq->retries = MAX_RETRIES;
...
#if defined (CONFIG_MIPS_BCM_NDVD)
	/*
	** For Mechanism Status, limit retries.
	*/
	if (SCpnt && (SCpnt->cmnd[0] == MECHANISM_STATUS))
		rq->retries = SCpnt->allowed;

	/*
	 * Make sure timeout conforms to SR_TIMEOUT
	 */
	if (rq->timeout < SR_TIMEOUT)
		rq->timeout = SR_TIMEOUT;
#endif
...
static int sr_probe(struct device *dev)
{
	...
	{
		/*
		 * Set up the default sr sectorsize
		 * for the blk layer.
		 */
		struct request_queue *queue;
		queue = cd->device->request_queue;
		blk_queue_hardsect_size(queue, sdev->sector_size);
	}
#endif
