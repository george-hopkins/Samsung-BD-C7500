...
int sr_do_ioctl(Scsi_CD *cd, struct packet_command *cgc)
{
	...
	if (cgc->buflen && (cgc->buflen < 8 || (cgc->buflen & 0x3)))
#else
	if (cgc->buflen & 0x3)
#endif
	{
...
#if defined (CONFIG_BCM_VIDE_OFE) && defined (CONFIG_MIPS_BCM7635)
		/*
		 * BCM763x OFE core has a bug where it cannot
		 * reliably perform 4-byte PIO reads. Therefore,
		 * make the bounce buffer at least 8 bytes to avoid
		 * the problem altogether.
		 */
		bounce_len = (user_len < 8) ? 8 :
			    ((user_len + 3) & ~0x3);
#else
