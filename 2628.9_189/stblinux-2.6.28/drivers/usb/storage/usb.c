...
static int usb_stor_control_thread(void * __us)
{
	...
	for(;;) {
		...
		if (test_bit(US_FLIDX_DISCONNECTING, &us->dflags)) {
			...
			while (us->srb_head) {
			    sreq = us->srb_head;
			    us->srb_head = sreq->next;
			    sreq->next = (struct srb *) 0;
			    us->srb = sreq->scmd;

			    sreq->scmd->result = DID_NO_CONNECT << 16;
			    sreq->scmd->scsi_done(sreq->scmd);
			    kfree(sreq);
			}
			spin_unlock_irqrestore( &us->srb_lock, irql);
...
#ifdef CONFIG_MIPS_BCM_NDVD
	us->srb_head = (struct srb *)0;
	us->srb_tail = (struct srb *)0;
	spin_lock_init( &us->srb_lock);
#endif
