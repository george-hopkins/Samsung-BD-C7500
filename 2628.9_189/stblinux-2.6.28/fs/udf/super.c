...
static int udf_vrs(struct super_block *sb, int silent)
{
	...
	for (; !nsr02 && !nsr03; sector += sectorsize) {
		...
		if ((t2.tv_sec - t1.tv_sec) > 10) {
			printk(KERN_WARNING
			       "\nudf_vrs() - Timer expiry, NO VRS FOUND, break at sector %d!\n",
			       (int)sector);
			break;
		}
#endif
...
static int udf_check_anchor_block(struct super_block *sb, sector_t block)
{
	...
	udf_debug("%s: Checking block %d\n",
		  __FUNCTION__, (int)block);

	if (UDF_QUERY_FLAG(sb, UDF_FLAG_VARCONV) &&
	    udf_fixed_to_variable(block) >=
#if defined (CONFIG_MIPS_BCM_NDVD)
	    sb->s_bdev->bd_disk->part0.nr_sects
#else
	...
	if (!bh && ((t2.tv_sec - t1.tv_sec) > 2)) {
		timeouts++;
		udf_debug("%s: Timeout error reading block %d\n",
			  __FUNCTION__, (int)block);
	}
	else if (!bh) {
		read_errors++;
		udf_debug("%s: Error reading block %d\n",
			  __FUNCTION__, (int)block);
	}
#endif
...
retry:
	...
	for (i = 0; i < ARRAY_SIZE(last); i++) {
		if (last[i] < 0)
			continue;
#if defined (CONFIG_MIPS_BCM_NDVD)
		if (last[i] >= sb->s_bdev->bd_disk->part0.nr_sects)
#else
...
#if defined (CONFIG_MIPS_BCM_NDVD)
		/*
		 * Fast track the failure if we timed out
		 * trying to read the last block of the
		 * media. If so, no other read based off
		 * of the last block will likely succeed.
		 * This speeds up successful mounting of
		 * media that the drive has a problem
		 * tracking to, or defects on the outer
		 * spirals.
		 */
		if ( (i == 0 && timeouts) ||
		     (timeouts > 2) ||
		     (read_errors > 12)) {
			udf_debug("%s: Error limit, BREAK!\n",
				  __FUNCTION__);
...
#if defined (CONFIG_MIPS_BCM_NDVD)
	/*
	 * If lastblock == 0 and all reads in the loop
	 * above timed or errored out, then retry with
	 * lastblock from READ_CAPACITY.
	 */
	if (lastblock == 0 &&
	    (timeouts == 2 || read_errors == 2)) {
		    udf_debug("%s: Current lastblock not working, RETRY!\n",
			      __FUNCTION__);
		    lastblock = udf_read_capacity(sb);
		    timeouts  = read_errors = 0;
		    goto retry;
	}
#endif
...
static void udf_find_anchor(struct super_block *sb)
{
	...
	udf_debug("%s: Called with lastblock = 0x%08x\n",
		  __FUNCTION__, sbi->s_last_block);
