...
static int yaffs_ReadChunkDataFromObject(yaffs_Object * in, int chunkInInode, __u8 * buffer)
{
    ...
    if (tn) {
        ...
        for (j = 0; theChunk && j < dev->chunkGroupSize; j++) {
            if (yaffs_CheckChunkBit(dev, theChunk / dev->nChunksPerBlock, theChunk % dev->nChunksPerBlock)) {
                int result = yaffs_ReadChunkWithTagsFromNAND(dev, theChunk, buffer, &tags);
                if (yaffs_TagsMatch(&tags, in->objectId, chunkInInode)) {
                    /* found it; */
                    return result;
                }
            }
    ...
    T(YAFFS_TRACE_NANDACCESS, (TSTR("Chunk %d not found zero instead" TENDSTR), chunkInInode));
...
int yaffs_GutsInitialise(yaffs_Device *dev)
{
	...
	if ((!dev->inbandTags && dev->isYaffs2 && dev->totalBytesPerChunk < YAFFS_MIN_YAFFS2_CHUNK_SIZE) ||
