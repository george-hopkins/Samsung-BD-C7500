...
#if !defined (CONFIG_MIPS_BCM_NDVD)	/* see comments below */
		else
...
static int __init ic_dynamic(void)
{
	...
	printk(KERN_NOTICE "IP-Config: Got %s answer from %u.%u.%u.%u, my address is %u.%u.%u.%u\n",
...
static int __init ip_auto_config(void)
{
 ...
 try_try_again:
#endif
#if defined (CONFIG_MIPS_BCM_NDVD)
	/* bcmemac driver init is now an async kernel thread that yields.
	 * Therefore, the driver is likely to be in the middle of MII
	 * line autoneg.
	 */
	for (i = 0; i < 4000; i++) {
		if (ic_open_devs() < 0)
	...
	printk(KERN_NOTICE "     device=%s, addr=%u.%u.%u.%u, mask=%u.%u.%u.%u, gw=%u.%u.%u.%u,\n",
	       ic_dev->name, NIPQUAD(ic_myaddr), NIPQUAD(ic_netmask), NIPQUAD(ic_gateway));
	printk(KERN_NOTICE "     host=%s, domain=%s, nis-domain=%s,\n",
	       utsname()->nodename, ic_domain, utsname()->domainname);
	printk(KERN_NOTICE "     bootserver=%u.%u.%u.%u, rootserver=%u.%u.%u.%u, rootpath=%s\n",
	       NIPQUAD(ic_servaddr), NIPQUAD(root_server_addr), root_server_path);
#else
	/*
	 * Clue in the operator.
	 */
