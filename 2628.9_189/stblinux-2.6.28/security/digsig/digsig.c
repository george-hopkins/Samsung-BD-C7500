...
static inline int is_unprotected_file(struct file *file)
{
	...
	/*
	 * Allowed local filesystems, in order of likelyhood.
	 *  squashfs
	 *  yaffs2
	 *  jffs2
	 *  yaffs
	...
	fs_name = file->f_dentry->d_inode->i_sb->s_type->name;

	if (strcmp(fs_name, "squashfs") == 0)
		return 0;

	if (strcmp(fs_name, "yaffs2") == 0)
		return 0;

	if (strcmp(fs_name, "jffs2") == 0)
		return 0;

	if (strcmp(fs_name, "yaffs") == 0)
		return 0;
	...
	if (strcmp(fs_name, "vfat") == 0)
		return 0;

	if (strcmp(fs_name, "msdos") == 0)
		return 0;


	/* 
	 * Every other filesystem fails.
	 * Network file systems are not secure due to  digsig cacheing of signatures.
	 */
	printk(KERN_ERR "DigSig: execution attempted from prohibited unsecure %s filesystem\n",
			fs_name);
	return 1;
}
