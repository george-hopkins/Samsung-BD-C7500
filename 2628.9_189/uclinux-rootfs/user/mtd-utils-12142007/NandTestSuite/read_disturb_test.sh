#The read disurb test consist in:
#   1- Erase 3 partitions A,B,C (contiguous)
#   2- Get a dump of A and C and keep it
#   3- Write to partition B
#   4- Read partition B
#   5- Dump partition A and C
#   6- Compare against original erased partition, they should still be erased. If not, a read
#      disturbance occurs.
#   7- If no error, Go back to step 4
...
#BAD_BLOCK_READ_DISTURB_TESTS_FILE
...
do
    echo 'executing Read Disturb Test - COUNTER:' $NAND_READWRITE_TESTS_COUNTER >> $FILE_RESULTS;
    echo 'executing Read Disturb Test - COUNTER:' $NAND_READWRITE_TESTS_COUNTER;   

    let NAND_READWRITE_TESTS_COUNTER--
    nandinfo -f $BAD_BLOCK_READ_DISTURB_TESTS_FILE >null
    
    # read partition B

    echo
    echo ">> nanddump -o -b -f $ROOT_RESULTS/$DISTURB2 $PARTITION_TWO_NAME"
    nanddump -o -b -f $ROOT_RESULTS/$DISTURB2 $PARTITION_TWO_NAME
    ...
    if test "$?" != "0"; then
        echo "Read Disturb TEST FAILED"  >>$FILE_RESULTS 
    ...
    echo "Read Disturb TEST SUCCESS"  >>$FILE_RESULTS;
