...
#ifdef USE_READLINE
	if (talking && isatty(ap->afile)) {
		static char *mycommand = NULL;
		static int pos = 0, size = 0;
		if (pos > size || !mycommand) {
			mycommand = readline(current_prompt);
			if (!mycommand) {
				closef(ap->afile);
				myputc('\n');
				return 0;
			}
			add_history(mycommand);
