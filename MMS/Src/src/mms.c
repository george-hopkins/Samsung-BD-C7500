...
/*   lprintf("%d\n", fallback_io_select(data, socket, MMS_IO_READ_READY, 1000)); */
  ...
  while (len < num)
  {
    if(!select(socket+1, &set, NULL, NULL, &tv))
    {
  		ret = 0;
    }else{
...
/*     fprintf(stderr, "failed to create socket: %s", strerror(errno)); */
  ...
  if (fcntl (s, F_SETFL, fcntl (s, F_GETFL) |O_NONBLOCK) == -1) {	//non-block
...
static int get_guid (unsigned char *buffer, int offset) {
  ...
  memcpy(&(g.Data1), buffer + offset, 4) ;  
  //g.Data2 = LE_16(buffer + offset + 4);
  memcpy(&(g.Data2), buffer + offset + 4, 2) ;	
  //g.Data3 = LE_16(buffer + offset + 6);
  memcpy(&(g.Data3), buffer + offset + 6, 2) ;
  for(i = 0; i < 8; i++) {
...
static void interp_asf_header (mms_t *this) {
  ...
  while (i < this->asf_header_len) {
    ...
    memcpy(&length, this->asf_header + i, 8) ;	
    i += 8;
    ...
    switch (guid) {
      ...
      case GUID_ASF_FILE_PROPERTIES:
        ...
        memcpy(&(this->asf_packet_len), this->asf_header + i + 92 - 24, 4) ;		
        ...
        memcpy(&(this->file_len), this->asf_header + i + 40 - 24, 8) ;	
        //this->time_len       = LE_64(this->asf_header + i + 64 - 24);
        memcpy(&(this->time_len), this->asf_header + i + 64 - 24, 8) ;
        //this->time_len       = LE_64(this->asf_header + i + 72 - 24);
        //this->preroll        = LE_64(this->asf_header + i + 80 - 24);
        memcpy(&(this->preroll), this->asf_header + i + 80 - 24, 8) ;
	  ...
	  uint16_t flags;
	    ...
	    case GUID_ASF_DEGRADABLE_JPEG_MEDIA:
          ...
          memcpy(&flags, this->asf_header + i + 48, 2) ;	  
      ...
      case GUID_ASF_STREAM_BITRATE_PROPERTIES:
        {
          ...
          uint16_t streams; 
          memcpy(&streams, this->asf_header + i, 2) ;		  
          uint16_t stream_id;
          int j;
          ...
          for(j = 0; j < streams; j++) {
            ...
            memcpy(&stream_id, this->asf_header + i + 2 + j * 6, 2) ;		
            lprintf ("stream id %d\n", stream_id);
            ...
            if (stream_index < this->num_stream_ids) {
              ...
              memcpy(&(this->streams[stream_index].bitrate), this->asf_header + i + 4 + j * 6, 4) ;	
      ...
      case GUID_ASF_DATA:
        ...
        memcpy(&this->asf_num_packets, this->asf_header + i + 40 - 24, 8) ;	
        break;
          ...
          case 0x1e:
            {
              ...
              memcpy(&error_code, this->buf + CMD_HEADER_LEN, 4) ;	  
