...
#define MAX_PROBE_PACKETS 100
    int probe_packets;
//sunye add language list ***************************************

#define MAX_LANG_DESC 20					//sunye add
    struct _Languagelist //sunye add this structure to pass the page id out of the ffmpeg start ************
    {
        char language[4];
        char sub_type;
        unsigned short comp_page_id;
        unsigned short anc_page_id;
    } lang_list[MAX_LANG_DESC];	//sunye add
    int lang_count;							//sunye add
//sunye add end**********************************************
//    int search_timestamp_from_bitrate;
    int nb_original_index_entries; // for reduce index entries in avi demuxer // cherry
    /**
     * Average framerate
     */
...
//Uniplayer_DRM	/*by uniplayer for Divx DRM*/
typedef struct DRMInfoList
{
    int DRM_Key_Index;
    int DRM_Offset;
    int DRM_Length;
} DRMInfoList;
///*by uniplayer for Divx DRM*/
...
//Uniplayer_DRM	/*by uniplayer for Divx DRM*/
    unsigned char Is_This_DRM_File;
    unsigned char Read_point;
    unsigned char Write_point;
    DRMInfoList DRM_information[40];
///*by uniplayer for Divx DRM*/

//for WIDEVINE
    unsigned char widevineFlag;
//WIDEVINE
//for VUDU
    unsigned char VUDUFlag;
//VUDU

/*
buffer pointer to use read packet when av_find_stream_info() by mcseo 20091002
*/
	unsigned char *pucTempPacketBuffer;
	int	iTempPacketSize;

//*********for NetFlix
    unsigned char NetFlixFlag;//*********for NetFlix

///*by uniplayer for SAMSUNG(Lee Lee Nam Author) Divx DRM*/
	unsigned char samsung_divx;
...
//for WIDEVINE//**********************************************for widevine
/** This function is just for WideVine
 * Change widevine playing speed to what you wants
 * @param Speed 
 * @return 1 if a valid numbered sequence string, 0 otherwise
 */
