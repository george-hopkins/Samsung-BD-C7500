...
int is_valid_indextable(AVStream *st, int64_t wanted_timestamp);
#undef NDEBUG
//#include <assert.h>
...
//s->streams[asf->stream_index]->codec->codec_type == CODEC_TYPE_AUDIO, asf->packet_obj_size);
        ...
        if (asf_st->frag_offset == asf_st->pkt.size) {
			...
			if(offset + asf_st->ds_chunk_size >  asf_st->pkt.size) {
				return AVERROR_INVALIDDATA;
			}
			 //assert(idx+1 <= asf_st->pkt.size / asf_st->ds_chunk_size);
			if(idx+1 > asf_st->pkt.size / asf_st->ds_chunk_size) {
				return AVERROR_INVALIDDATA;	
			}
...
static void asf_build_simple_index(AVFormatContext *s, int stream_index)
{
	...
	int64_t previous_pos =0;
	    ...
	    if(previous_pos == pos) {
            	av_add_index_entry(s->streams[stream_index], pos, index_pts, asf->packet_size, 0, 0);
	    } else {
...
#ifdef MMS_PROTO
extern int MMSTimeSeek(double time_sec);
#endif
#ifdef WMDRM//###################################for WMDRM
extern int NetFlix_connect;
extern int IPlayer_connect;
#endif
...
#ifdef MMS_PROTO
	double time_sec;
	if(!strcmp(url_fileno(s->pb)->prot->name , "mms")) {
		av_log(NULL,AV_LOG_INFO,"[asf_read_seek] Request MMS TimeSeek PTS[%lld]\n" , pts);
		time_sec = (double)(pts * st->time_base.num / st->time_base.den);
		av_log(NULL,AV_LOG_INFO,"[asf_read_seek] TIME_SEC [%f]\n" , time_sec);	
		pos = (int64_t)MMSTimeSeek(time_sec);
		av_log(NULL,AV_LOG_INFO,"[asf_read_seek] MMSTimeSeek RET [%lld] \n" , pos);
		// move pos 
		 url_fseek(s->pb, pos, SEEK_SET);
		 asf_reset_header(s);	
...
#if 0
    av_log(NULL,AV_LOG_INFO,"\n\n[%s][%d] asf->index_read[%d] ,stream_index[%d] st->nb_entries[%d]\n\n",__FUNCTION__,__LINE__,asf->index_read,stream_index,st->nb_index_entries);
    if(st->index_entries) {
        av_log(NULL,AV_LOG_INFO,"\n st->index_entries is NOT NULL!!\n",st->index_entries);
    } else {
        av_log(NULL,AV_LOG_INFO,"\n st->index_entries is NULL!!\n",st->index_entries);
    }
...
#ifdef WMDRM//###################################for WMDRM
	if(NetFlix_connect == 1 || IPlayer_connect == 1){
		av_log(NULL,AV_LOG_INFO,"\n[%s][%d] Call av_index_search_timestamp..  \n",__FUNCTION__,__LINE__);
...
#endif
    if(!(asf->index_read && st->index_entries) || !is_valid_indextable(st , s->duration * s->streams[stream_index]->time_base.den / s->streams[stream_index]->time_base.num)){
       av_log(NULL,AV_LOG_INFO,"\n[%s][%d] Call av_seek_frame_binary..  \n",__FUNCTION__,__LINE__);
...
int is_valid_indextable(AVStream *st, int64_t  wanted_timestamp)
{
    ...
    {
        if(wanted_timestamp > (st->index_entries[st->nb_index_entries - 1].timestamp * 2))
        {
            return 0;
        }
    }
...
extern int asf_seek_available(AVFormatContext *s, int stream_index, int64_t wanted_timestamp, int flags)
{
    ...
    if(s->priv_data == NULL) 
    {
        av_log(NULL,AV_LOG_INFO,"Something is wrong .. [%s][%d]\n",__FUNCTION__,__LINE__);
        return -1;        
    }
    ...
    //int64_t current_position = url_ftell(s->pb);
        
    av_log(NULL,AV_LOG_INFO,"[%s][%d] STREAM INDEX = %d, wanted_timestamp=%lld\n",__FUNCTION__,__LINE__, stream_index, wanted_timestamp);

    int64_t fsize = url_fsize(s->pb);
    if(fsize < ((int64_t)1<<28))  // 256MB
    {
        av_log(NULL,AV_LOG_INFO,"\nFile Size IS under 128MB so Can try seek\n");
        return 0;      
    } else {
    if (!asf->index_read) {
        av_log(NULL,AV_LOG_INFO,"[%s][%d] Loading asf simple index..\n",__FUNCTION__,__LINE__);
        asf_build_simple_index(s, stream_index);
    }
    
        // if file size is big than check index table is or not
   if(!(asf->index_read && st->index_entries))
   {
        av_log(NULL,AV_LOG_INFO,"[%s][%d] Hmm... There is no index table. So Check FileSize...\n",__FUNCTION__,__LINE__);
            return -1;
        } else {

            // if there is index table, check index table is valid or not                
        if(!is_valid_indextable(st , wanted_timestamp))
            {
                //invalid
                av_log(NULL,AV_LOG_INFO,"[%s][%d]  invalid indextable \n",__FUNCTION__,__LINE__);
                return -1; // cannot seek
        }
            // try go to the end , and check success or not
        av_log(NULL,AV_LOG_INFO,"[%s][%d]  Try index search timestamp : target-ts [%lld]\n",__FUNCTION__,__LINE__, wanted_timestamp);
        index= av_index_search_timestamp(st, wanted_timestamp, flags);
        if(index<0) {
            av_log(NULL,AV_LOG_INFO,"[%s][%d]  Can't find index in table \n",__FUNCTION__,__LINE__);
            return -1;
        }
    }
    }
