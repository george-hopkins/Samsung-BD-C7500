...
int64_t url_fseek(ByteIOContext *s, int64_t offset, int whence)
{
    ...
    if (!s->must_flush &&
        ...
        if (!s->seek || (res = s->seek(s->opaque, offset, SEEK_SET)) < 0){
            if(res == -99) {
		av_log(NULL,AV_LOG_INFO,"set s->error = -99 to handle network disconnect\n");
		s->error  = res;//added by chun jung sik for disconnected Network
            }
            return res;
        }
        s->pos = offset;
...
//Widevine_Moon
	if (!strcmp(url_fileno(s)->prot->name , "wvine") && url_fileno(s)->server_ret_value == -1000) {
		return;
	}
//Widevine_Moon
