                              ...
                              int64_t timestamp, int flags)
{
    ...
    av_log(NULL,AV_LOG_INFO,"[%s][%d] timesamp  =%lld , first_dts = %lld \n",__FUNCTION__ ,__LINE__ ,timestamp,st->first_dts);
	...
	av_log(NULL,AV_LOG_INFO,"[%s][%d]search timestamp is failed \n",__FUNCTION__,__LINE__);		
    ...
    av_log(NULL,AV_LOG_INFO,"[%s][%d] index  =%d searched timestamp = %lld ,pos = %lld \n",__FUNCTION__ ,__LINE__ ,index ,st->index_entries[index].timestamp, st->index_entries[index].pos);
    matroska_clear_queue(matroska);
    if (index < 0)
        return 0;
    ...
    av_log(NULL,AV_LOG_INFO,"[%s][%d] Call url_fseek : pos = %lld \n",__FUNCTION__,__LINE__,st->index_entries[index_min].pos );
    url_fseek(s->pb, st->index_entries[index_min].pos, SEEK_SET);
    matroska->skip_to_keyframe = !(flags & AVSEEK_FLAG_ANY);
    matroska->skip_to_timecode = st->index_entries[index].timestamp;
    matroska->done = 0;
    av_log(NULL,AV_LOG_INFO,"[%s][%d] update cur dts : st->index_entries[index].timestamp = %lld \n",__FUNCTION__,__LINE__,st->index_entries[index].timestamp );	
...
/**
    ...
    return 1; valid
*/
int is_valid_mkv_indextable(AVStream *st , int64_t wanted_timestamp)
{
    ...
    av_log(NULL,AV_LOG_INFO,"[%s][%d] mkd index table entries = %d \n",__FUNCTION__,__LINE__,st->nb_index_entries);
    av_log(NULL,AV_LOG_INFO,"[%s][%d] estimated last timestamp = %lld \n",__FUNCTION__,__LINE__,wanted_timestamp);	

    if((st->codec != NULL) && (st->codec->codec_type == CODEC_TYPE_VIDEO) && (st->nb_index_entries < 5))
    {
        return 0; // invalid
    }
    ...
    if((wanted_timestamp >0) && (wanted_timestamp != AV_NOPTS_VALUE) && st->nb_index_entries > 0)
    {
        ...
        {
	    av_log(NULL,AV_LOG_INFO,"[%s][%d] mkv index table is broken \n",__FUNCTION__,__LINE__);
            return 0;
        }
    }


/** Compare with Audio index Table..
*/



/** Count size=0 index entries, useless in mkv.
...
extern int mkv_seek_available(AVFormatContext *s, int stream_index, int64_t wanted_timestamp, int flags) 
{
    ...
    if(!is_valid_mkv_indextable(s->streams[stream_index] , wanted_timestamp))
    {
