...
#if 0
// DLNA에서 5M_Australia.3gp 파일 재생시 EOF 위치에서 잘못된 stream_index의 패킷이 계속 올라오는 문제 있어서 삭제함. (2010. 02.09)
#ifdef __TRIDENT_SPECIFIC
    if(!strcmp(url_fileno(pb)->prot->name , "dlna"))
    {
	av_log(NULL,AV_LOG_INFO,"[%s][%d] Trident DLNA environment.. set streaming.. test ver 0.2\n",__FUNCTION__,__LINE__);
	pb->is_streamed = 1;
    }
#endif
#endif
...
/**
    ...
    return 1; valid
*/
int is_valid_mov_indextable(AVStream *st , int64_t wanted_timestamp)
{
...
/* Count KeyFrame .*/
    ...
    int i;
    for(i=1; i<st->nb_index_entries; i++)
    {
        if(st->index_entries[i].flags == 1) {
            nb_valid_index++;
        }
    }
...
extern int mov_seek_available(AVFormatContext *s, int stream_index, int64_t wanted_timestamp, int flags)
{
    ...
    if(!is_valid_mov_indextable(s->streams[stream_index] , wanted_timestamp))
    {
