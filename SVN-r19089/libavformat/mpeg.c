...
//Widevine_Moon
	if (!strcmp(url_fileno(pb)->prot->name , "wvine")) {
		if(WV_OldSpeed != av_WV_getSpeed()) {
			url_fileno(pb)->server_ret_value = 0;
		}

		if (url_fileno(pb)->server_ret_value == -1000) { 
			break;
		}		
	}
//Widevine_Moon
...
//Widevine_Moon
	if (!strcmp(url_fileno(s->pb)->prot->name , "wvine")) {
		if(WV_OldSpeed != av_WV_getSpeed()) {
			WV_OldSpeed  = 	av_WV_getSpeed();
			url_fileno(s->pb)->server_ret_value = 0;
		}

		if (url_fileno(s->pb)->server_ret_value == -1000)
		{ 
                              ...
                              AVPacket *pkt)
{
 ...
 redo:
    ...
    //av_log(NULL,AV_LOG_INFO,"[ffmpeg][mpeg.c] mpegps_read_packet ) es_type = 0x%x\n", es_type);
    if(es_type > 0 && es_type != STREAM_TYPE_PRIVATE_DATA){
	    ...
	    av_log(NULL,AV_LOG_INFO,"\n[ffmpeg][mpeg.c] CODEC_ID_PROBE__\n");
