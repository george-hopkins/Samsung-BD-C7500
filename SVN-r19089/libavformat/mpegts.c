...
/*! security information for freesat spec,  
      ATTENTION: the definition of the structure here must be identical to the definition outside!!!!
      clin.li@SCRC for iplayer application, 2009.9.29 
*/
#define MAX_KEY_SLOTS    20
struct security_info
{
    unsigned int length;  /*< for checking the validaty of this structure to keep dangers away */
    void (*decipher)(unsigned char *pData, int size, unsigned char *pOut, unsigned char key[16], unsigned char iv[16]);
    unsigned int nb_keys;
    struct
    {
        unsigned int pid;
        unsigned char key_even[16];
        unsigned char key_odd[16];
        unsigned char iv[16];
    } keys[MAX_KEY_SLOTS];        
};
/*! end clin.li */
...
struct MpegTSContext {
    /* user data */
    struct security_info user_data; /*< user data field for security information, clin.li@SCRC for iplayer application, 2009.9.29 */
...
static const StreamType MISC_types[] = {
    ...
    { 0x87, CODEC_TYPE_AUDIO,   CODEC_ID_EAC3 },
    { 0 },
};
...
static void pmt_cb(MpegTSFilter *filter, const uint8_t *section, int section_len)
{
    ...
    /*! dvb stream identifier (Component Tag), clin.li@SCRC for iplayer application, 2009.10.1 */	
    int i; //< iterator
    uint8_t stId[2] = {0, 0}; //<avmetatdata API check key length by "strlen", so it's better to add '\0' after component tag 
    /*! end clin.li */
    ...
    for(;;) {
        ...
        for(;;) {
            ...
            case 0x59: /* subtitling descriptor */
                /*! for multi-lang subtitle descriptor, clin.li@SCRC for iplayer application, 2009.10.1 */
                for(i = 0; (i < desc_len / 8) && (i< MAX_LANG_DESC); i++)			//sunye add maybe the subtitle have more then one descripter in the multi-language situation and 1 descaptor has 8 byte
                {
                    language[0] = get8(&p, desc_end);
                    language[1] = get8(&p, desc_end);
                    language[2] = get8(&p, desc_end);
                    language[3] = 0;
                    st->lang_list[i].sub_type = get8(&p, desc_end);			//sunye add get sub type
                    get8(&p, desc_end);
                    comp_page = get16(&p, desc_end);
                    anc_page = get16(&p, desc_end);
                    st->codec->sub_id = (anc_page << 16) | comp_page;
                    st->lang_list[i].language[0] = language[0];				//sunye add				
                    st->lang_list[i].language[1] = language[1];				//sunye add
                    st->lang_list[i].language[2] = language[2];				//sunye add				
                    st->lang_list[i].language[3] = language[3];				//sunye add
                    st->lang_list[i].comp_page_id = comp_page;			//sunye add
                    st->lang_list[i].anc_page_id = anc_page;				//sunye add
                }													//sunye add
                av_metadata_set(&st->metadata, "language", language);
                st->lang_count = i;													//the number of lang_desc                
                break;
            case 0x0a: /* ISO 639 language descriptor */
                ...
                language[3] = 0;
                av_metadata_set(&st->metadata, "language", language);
                 /*! audio type, clin.li@SCRC for iplayer application, 2009.10.1 */
                stId[0] = get8(&p, desc_end);
                av_metadata_set(&st->metadata, "audio_type", stId);
                 /*! end clin.li */                
                break;
            ...
            case 0x52:	/* DVB Stream Identifier */				
                stId[0] = get8(&p, desc_end);				
                av_metadata_set(&st->metadata, "DVB_STREAM_IDENTIFIER", stId);		
                break;				
            /*! end clin.li */                
            default:
                break;
...
static void new_pes_packet(PESContext *pes, AVPacket *pkt)
{
    ...
    /*! fill PES header, clin.li@SCRC for iplayer application, 2009.10.9 */
    memcpy(pkt->header, pes->header,  pes->pes_header_size); //sunye add 	
    /*! end clin.li */
...
/*! clin.li@SCRC for iplayer application, 2009.9.29  */
static void decrypt_packet(MpegTSContext *ts, uint8_t *packet)
{
    struct security_info *px = &(ts->user_data);
    unsigned char buf[TS_PACKET_SIZE];
    unsigned char *p1;
    int size;
    int pid, id;

   if (px->length != sizeof(struct security_info))  /*< guarantee the validaty of the field to keep danger away */
   {
        av_log(NULL, AV_LOG_ERROR, "INVALID SECURITY INFORMATION!!! DANGEROUS......\n" );
        return;
   }

    p1 = packet + 4;
    size = TS_PACKET_SIZE - 4;
    if (packet[3]&0x20)  // skip adaptation field;
    {
        if (packet[3]&0x10)
        {
            p1 += packet[4]+1;
    ...
    while (id < px->nb_keys)
    {
        if (px->keys[id].pid == pid) break;
        id++;
    }
    if (px->keys[id].pid != pid) 
    {
        av_log(NULL, AV_LOG_DEBUG, "AES key for pid 0x%x not found!\n", pid); /*< key not found */
        return;
    }

    if (packet[3]&0x40)
        px->decipher(p1, size, buf, px->keys[id].key_odd, px->keys[id].iv);
    else
        px->decipher(p1, size, buf, px->keys[id].key_even, px->keys[id].iv);
...
static int handle_packets(MpegTSContext *ts, int nb_packets)
{
    ...
    for(;;) {
        ...
        /*! check decryption for freesat, clin.li@SCRC for iplayer application, 2009.9.29  */
        if (packet[3]&0x80)
        {
            decrypt_packet(ts, packet);
        }
        /*! end clin.li */
...
// we need a clear definition for the returned score otherwise things will become messy sooner or later
    if     (score > fec_score && score > dvhs_score && score > 3/*6*/) return AVPROBE_SCORE_MAX + score     - CHECK_COUNT;
                              ...
                              AVFormatParameters *ap)
{
    ...
    /*! initialize security information, clin.li@SCRC for iplayer application, 2009.9.30 */
    memset((void *)&(ts->user_data), 0, sizeof(struct security_info));
    /*! end clin.li */
    ...
    if (s->iformat == &mpegts_demuxer) {
        ...
        for(;;) {
            ...
            if (ret < 0) {
                mpegts_handle_exception = ret;
                av_log(NULL,AV_LOG_INFO,"[%s][%d] mpegts_handle_exception = %d \n",__FUNCTION__,__LINE__, mpegts_handle_exception);
                return -1;
            }
            pid = AV_RB16(packet + 1) & 0x1fff;
...
static int read_seek(AVFormatContext *s, int stream_index, int64_t target_ts, int flags){
    ...
    int iRet =0;
    if( (iRet = av_seek_frame_binary(s, stream_index, target_ts, flags)) < 0) {
        av_log(NULL,AV_LOG_INFO,"[%s][%d] av_seek_frame_binary failed ..!!RET[%d] \n",__FUNCTION__,__LINE__,iRet);
        if(iRet == -777) {
            return -777;
        }
        return -1;
