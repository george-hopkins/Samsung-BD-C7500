...
{
#ifdef BCMUSESLINUXINPUTS
     if ((dfb_system_type() == CORE_FBDEV || dfb_system_type() == CORE_BCM) && dfb_config->vt)
          return 1;
#else
                    ...
                    void            **driver_data )
{
     struct termios  ts;
     KeyboardData   *data;
     void           *system_data = dfb_system_data();
     ...
     switch (dfb_system_type()) {
         case CORE_FBDEV:
             data->vt     = ((FBDev *)system_data)->vt;
         break;
#ifdef BCMUSESLINUXINPUTS
         case CORE_BCM:
             data->vt     = ((bcmgraphics_context *)system_data)->vt;
         break;
#endif
         default:
             D_ERROR( "DirectFB/Keyboard: no vt for this system type!\n" );
             return DFB_INIT;
     }
