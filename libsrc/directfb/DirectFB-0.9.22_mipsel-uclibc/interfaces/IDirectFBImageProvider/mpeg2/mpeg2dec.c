...
static int video_sequence_with_scaling(MPEG2_Decoder *dec, int *Bitstream_Framenumber);
...
int
MPEG2_DecodeAndScale(MPEG2_Decoder *dec, MPEG2_Write mpeg2_write, void *write_ctx, MPEG2_Scale mpeg2_scale)
{
     int Bitstream_Framenum = 0;

     dec->mpeg2_write = mpeg2_write;
     dec->mpeg2_write_ctx = write_ctx;
     dec->mpeg2_scale = mpeg2_scale;
     dec->get_out = 0;
     dec->video_drip_mode = 1;

     return video_sequence_with_scaling(dec, &Bitstream_Framenum);
}
...
static int
video_sequence_with_scaling(MPEG2_Decoder *dec, int *Bitstream_Framenumber)
{
     int Bitstream_Framenum;
     int Sequence_Framenum;
     int Return_Value = 0;
     ...
     if (!dec->Second_Field) {
      ...
      /* always show the first decoded picture */
     if (Sequence_Framenum!=0) {
          MPEG2_Output_Last_Frame_of_Sequence(dec, Bitstream_Framenum);
     }
     ...
     if (dec->mpeg2_scale)
     {
          dec->mpeg2_scale(dec->mpeg2_write_ctx);
     }

     /* loop through the rest of the pictures in the sequence */
     while ((Return_Value=MPEG2_Get_Hdr(dec)) && (dec->get_out==0)) {
          MPEG2_Decode_Picture(dec, Bitstream_Framenum, Sequence_Framenum);

          /*printf("decode another image\n");*/
     ...
     *Bitstream_Framenumber = Bitstream_Framenum;
     ...
     return(dec->get_out ? 0 : Return_Value);
}
