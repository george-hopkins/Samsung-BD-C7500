                                                  ...
                                                  void                   *ctx )
{
     ...
     if ( 0 != data->is_v4l2 ) {
          ...
          if ( getenv("DFB_V4L_GRAB") || surface->caps & DSCAPS_SYSTEMONLY
                ...
                * permanently lock it.
                */
               ret = dfb_surface_hardware_lock( surface, surface->back_buffer, 
                                                DSLF_WRITE | CSLF_FORCE );
...
static void* GrabThread( DirectThread *thread, void *ctx )
{
     ...
     while (data->running) {
          ...
          dfb_surface_soft_lock( surface, surface->back_buffer, DSLF_WRITE, &dst, &dst_pitch );
          while (h--) {
...
#endif
          if (!data->grab_mode)
               dfb_surface_unlock( destination, destination->back_buffer, 0 );
#ifdef DFB_HAVE_V4L2
     }
#endif
