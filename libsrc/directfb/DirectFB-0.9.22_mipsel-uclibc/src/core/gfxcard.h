...
typedef struct _GraphicsDeviceFuncs {
     ...
     void (*FlushSpecificTextureCache)( void *driver_data, void *device_data, SurfaceBuffer *buffer );
     ...
     bool (*WeighArea)( void *driver_data, void *device_data,
                        CardState *state, DFBAccelerationMask accel,
     ...
     bool (*GetBackFieldContext)( void *driver_data, void *device_data,
...
void dfb_gfxcard_drawstring( const __u8 *text, int bytes, int x, int y,
                             CoreFont *font, CardState *state, DFBSurfaceTextFlags textFlags);
...
void            dfb_gfxcard_get_limitations( CardLimitations *limits );

void            dfb_gfxcard_get_surface_management( void **funcs );
...
void dfb_gfxcard_surface_back_field_context(CoreSurface *surface, void **ctx);
