...
static void      screen_location     ( CoreLayerContext            *context,
                                      ...
                                      bool               context_locked )
{
     ...
     back_location = context->screen.location;
     ...
     if (ret == DFB_OK) {
          ...
          context->screen.location  = back_location;
     }
                                       ...
                                       const DFBRectangle *rectangle )
{
     ...
     if (context->screen.mode == CLLM_LOCATION) {
          DFBLocation location;
          /* Calculate new normalized screen coordinates. */
          screen_location( context, rectangle, &location);
          ret = dfb_layer_context_set_screenlocation( context, &location, true );
     }
     else {
          /* Do nothing if the location didn't change. */
                                      ...
                                      int                y )
{
     ...
     if (context->screen.mode == CLLM_LOCATION) {
          DFBLocation location;
          DFBRectangle rectangle = { x, y,
                                     context->primary.config.dest.w,
                                     context->primary.config.dest.h };
          /* Calculate new normalized screen coordinates. */
          screen_location( context, &rectangle, &location);
          ret = dfb_layer_context_set_screenlocation( context, &location, true );
     }
     ...
     *ret_config = context->primary.config;
     ...
     if (update->flags & (DLCONF_WIDTH | DLCONF_HEIGHT)) {
          ...
          switch (context->screen.mode) {
               ...
               case CLLM_LOCATION:
                    /* [xm] We need to calculate the new absolute screen coordinates.
                     * Else CLRCF_DEST is defined without dest changing...
                     */
                  ...
                  CoreLayerRegionConfig *config )
{
     ...
     if (funcs->AllocateSurface) {
          ...
          ret = dfb_surface_create( layer->core, NULL,
...
static void
screen_location( CoreLayerContext   *context,
                 const DFBRectangle *rect,
                 DFBLocation        *location )
{
     DFBResult  ret;
     ...
     if (ret) {
          ...
          location->x = (float)rect->x/(float)width;
          location->y = (float)rect->y/(float)height;
          location->w = (float)rect->w/(float)width;
          location->h = (float)rect->h/(float)height;
     }
}
