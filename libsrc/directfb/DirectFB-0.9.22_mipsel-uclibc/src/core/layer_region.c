                              ...
                              DFBSurfaceFlipFlags  flags )
{
     ...
     switch (region->config.buffermode) {
          ...
          case DLBM_BACKSYSTEM:
               if (region->config.buffermode == DLBM_BACKSYSTEM) {
                    ...
                    D_DEBUG_AT( Core_Layers, "  -> Skipping copy, driver will take care of that in UpdateRegion...\n" );
               }
               /* fall through */
          ...
          case DLBM_FRONTONLY:
               ...
               if (funcs->UpdateRegion && D_FLAGS_IS_SET( region->state, CLRSF_REALIZED )) {
                    ...
                    ret = funcs->UpdateRegionFlags( layer,
                                                    layer->driver_data,
                                                    layer->layer_data,
                                                    region->region_data,
                                                    surface, update, flags );
               }
               break;
