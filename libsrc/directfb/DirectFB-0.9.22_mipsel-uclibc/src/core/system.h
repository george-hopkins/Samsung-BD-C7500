...
typedef struct {
     ...
     void*          (*VideoMemoryVirtualFromPartition)( unsigned int offset, unsigned int type);
     unsigned int   (*GetPartitionType)( unsigned int Policy);
	 unsigned int   (*GetNumOfPartitions)();
     unsigned int   (*VideoRamLength)(unsigned int partition);
} CoreSystemFuncs;
...
unsigned int
dfb_system_get_partition_type( unsigned int policy);

unsigned int
dfb_get_num_of_partitions();

unsigned int
dfb_system_videoram_length( unsigned int partition );
