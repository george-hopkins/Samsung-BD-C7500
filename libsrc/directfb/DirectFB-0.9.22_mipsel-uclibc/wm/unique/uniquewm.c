...
load_foo( CoreDFB *core, WMShared *shared )
{
     ...
     ret = dfb_surface_create( core, NULL, foo_desc.width, foo_desc.height, foo_desc.pixelformat,
     ...
     ret = dfb_surface_soft_lock( shared->foo_surface, shared->foo_surface->back_buffer, DSLF_WRITE, &data, &pitch );
     if (ret) {
     ...
     dfb_surface_unlock( shared->foo_surface, shared->foo_surface->back_buffer, 0 );
