...
typedef struct {
	uint32 magic0xA5A0F5A5;	/* For basic sanity checking */
	int16 numberOfContours;	/* Number of contours in the character */
	int16 *startPoint;		/* should be set to point at the same field in the ag_ElementType structure */
	int16 *endPoint;		/* should be set to point at the same field in the ag_ElementType structure */
	uint8 *onCurve;			/* should be set to point at the same field in the ag_ElementType structure */
	...
	int16 isFigure; /* Set to true for 0..9 and false otherwise */
	...
	int16 ocvt[MAX_CVT_ENTRIES];	/* unscaled control values */
	int16 unitsPerEm;				/* Units per Em for the fonts */
	int32 xPixelsPerEm;				/* Pixels per em in the x direction */
	int32 yPixelsPerEm;				/* Pixels per em in the y direction */
	...
	int32  ttCodeBaseMaxLength;	/* The current allocated length */
	uint8 *ttCodeBase;			/* The start of the allocated memory */
	uint8 *ttcode;				/* The first free location */
	/* The data for the stack */
	int32  ttDataBaseMaxElements; /* The current allocated length */
	int16 *ttDataBase;			  /* The start of the allocated memory */
	int16 *ttdata;				  /* The first free location */
	/* The TrueType hint fragment */
	int32  hintFragmentMaxLength; /* The current allocated length */
	uint8 *hintFragment;		  /* The start of the allocated memory */
	int32  hintFragmentLength;	  /* The first free location */
	/* The TT graphics state */
	int16 RP0, RP1, RP2;		/* These mirror the TrueType local graphic state variables RP0,RP1, and RP2 */
#endif /* ENABLE_ALL_AUTO_HANDG_CODE */
...
#define MAXSTORAGE				16
	...
	int16 inX;	/* Set to true when we operate in the x direction and false otherwise */
	int16 inY;	/* Set to true when we operate in the y direction and false otherwise */
	int8 cvtHasBeenSetUp; /* Boolean indicating if the cvt has been initialized */
	int8 hintInfoHasBeenSetUp; /* Boolean indicating if the the hint info has been initialized by ag_SetHintInfo */
	tsiMemObject *mem;
	int strat98;
	uint32 magic0x0FA55AF0; /* For basic sanity checking */
} ag_DataType;
 ...
 *  
 *     $Header: /encentrus/Andover/juju-20060206-1447/ndvd/directfb/directfb_deps_source/FontFusion_3.1/FontFusion_3.1_SDK/core/agridint.h,v 1.1 2006/02/08 17:20:43 xmiville Exp $
 *                                                                           *
 *     $Log: agridint.h,v $
 ...
 *     *** empty log message ***
 *
 *     Revision 1.6  2003/01/03 18:03:34  Shawn_Flynn
 *     dtypED
 *     
